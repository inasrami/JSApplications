...
describe("E2E tests", function () {
    ...
    describe("Home Page [ 5 Points ]", function () {
        it("Show Home page text [ 2.5 Points ]", async function () {
            await page.goto(host);
            await page.waitForSelector('text=Peak performance on four wheels. Your ultimate destination for innovation, power, and an unparalleled driving experience.', { timeout: interval });
            expect(await page.isVisible("text=Peak performance on four wheels. Your ultimate destination for innovation, power, and an unparalleled driving experience.")).to.be.true;
        });

        it("Show home page image [ 2.5 Points ]", async function () {
            await page.goto(host);
            await page.waitForSelector('.hero-logo', { timeout: interval });
            expect(await page.isVisible('.hero-logo')).to.be.true;
        });
    });
